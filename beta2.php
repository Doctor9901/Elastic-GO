...
<head>
<meta charset="utf-8">
<title>Exercício 3 - Exercício com Polichinelo</title>
...
<div class="container py-5 text-center">
  ...
  <div class="header-exercicio mb-4">
    <h1 class="mb-0">🏋️ Exercício: Alogamento com Polichinelo</h1>
  ...
  <div class="main-container">
    ...
    <div class="video-section">
      ...
      <div class="demo-box">
        ...
        <img src="https://hips.hearstapps.com/hmg-prod/images/workouts/2016/03/jumpingjack-1457045563.gif?resize=1200:*" alt="Demonstração de Alongamento">
      </div>
...
// 📌 Lógica do movimento para cima e para baixo (CIMA / BAIXO)
...
async function analisarPostura() {
  ...
  if (poses.length > 0) {
    ...
    // === LÓGICA DE MOVIMENTO VERTICAL (BRAÇOS PARA CIMA / BAIXO) ===
      const maoDir = pose[10];  // Mão direita
      const maoEsq = pose[9];   // Mão esquerda
      const ombroDir = pose[6]; // Ombro direito
      const ombroEsq = pose[5]; // Ombro esquerdo
      const quadril = pose[12]; // Quadril

    if (maoDir && maoEsq && ombroDir && ombroEsq && quadril) {
      const acimaOmbroDir = maoDir.y < ombroDir.y;
      const acimaOmbroEsq = maoEsq.y < ombroEsq.y;

      const abaixoQuadrilDir = maoDir.y > quadril.y;
      const abaixoQuadrilEsq = maoEsq.y > quadril.y;

      if (acimaOmbroDir && acimaOmbroEsq) {
        linha.style.backgroundColor = "green"; // VERDE
        if (movimentoAnterior !== "cima") {
          falar("Muito bem! Braços e pernas movimentados corretamente!");
          movimentoAnterior = "cima";
        }
      }
      else if (abaixoQuadrilDir && abaixoQuadrilEsq) {
        linha.style.backgroundColor = "green"; // VERDE
        if (movimentoAnterior !== "baixo") {
          falar("Muito bem! Branços e pernas movimentados corretamente!");
          movimentoAnterior = "baixo";
        }
      }
      else {
        linha.style.backgroundColor = "red"; // VERMELHO
        if (movimentoAnterior !== "errado") {
          falar("Movimente mais os braços e pernas!");
