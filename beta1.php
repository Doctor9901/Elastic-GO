...
<head>
<meta charset="utf-8">
<title>Exercício 1 - Caminhada</title>
...
.video-section {
  ...
  justify-content: center;
  align-items: flex-start;
  gap: 150px;   /* 🔥 AQUI AUMENTAMOS O ESPAÇO */
  flex-wrap: nowrap;
  position: relative;
}
...
canvas {
  ...
  top: 0;
  left: 0;
  width: 600px;     /* mesmo tamanho do canvas */
  height: 500px;    /* aumenta altura = aparece mais perna */
  z-index: 2;
  pointer-events: none;
...
<div class="container py-5 text-center">
  ...
  <div class="header-exercicio mb-4">
    <h1 class="mb-0">🏃 Exercício: Caminhada</h1>
  ...
  <div class="main-container">
    ...
    <div class="video-section">
      ...
      <div class="demo-box">
        ...
        <img src="https://www.bing.com/th/id/OGC.bdc18ad595351b692449cbba66f3b2bd?o=7&pid=1.7&rm=3&rurl=https%3a%2f%2ftreinamentofuncional.net.br%2fwp-content%2fuploads%2f2017%2f09%2f12.gif&ehk=D27X9sM4UPFLEAt5bZN8AzWIV4KDeOsSRdfJQ32e2WE%3d" alt="Demonstração do exercício">
      </div>
      ...
      <div class="botoes-laterais">
        ...
        <form method="POST" action="resultado.php" id="formExercicio">
          <input type="hidden" name="exercicio" value="Caminhada">
...
// === Ligações do esqueleto (osso → cor e espessura) ===
const bones = [
  // cabeça
...
function desenharCabeca(p0, p1) {
  ...
  ctx.fillStyle = "#ffe680";    // cor da cabeça cartoon
  ...
  if (poses.length > 0) {
    ...
    bones.forEach(({ a, b, color, width }) => {
      ...
      if (p1.score > 0.5 && p2.score > 0.5) {
        ...
        ctx.lineWidth = width;
        ctx.lineCap = "round";   // cartoon suave
        ctx.stroke();
      }
    });

    // === SUA LÓGICA DE POSTURA ===
    const quadrilDir = pose[12];
    const joelhoDir = pose[14];

    if (quadrilDir.score > 0.5 && joelhoDir.score > 0.5) {
      const linhaY = linha.offsetTop;  // posição Y da linha

      if (joelhoDir.y < linhaY) {
        linha.style.backgroundColor = "green";
        falar("Muito bem! Continue assim.");
      } else {
        linha.style.backgroundColor = "red";
        falar("Postura incorreta, tente levantar mais o joelho.");
      }
    }
  }
...
iniciarBtn.onclick = async () => {
  ...
  falar("Bem-Vindo ao exercício, antes de começar siga as orientações para ter um ótimo desempenho, primeiro ajuste a câmera para aparecer a cabeça até o joelho. Dois fique visivel de frente para câmera com uma distância de 2 a 4 passos. Você tem 10 segundos para se preparar, contando. um. dois. três. quatro. cinco. seis. sete. oito. nove. dez. Iniciando caminhada. Caminhe no lugar devagar e mantenha a postura como na demostração.");
  resultado.textContent = "🏃 Exercício iniciado...";
  analisarPostura();
